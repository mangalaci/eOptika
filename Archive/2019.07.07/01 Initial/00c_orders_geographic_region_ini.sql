...
SELECT m.erp_id,
			MAX(CASE 		WHEN s.user_type = 'Private insurance' THEN 'Private insurance'
							...
							ELSE s.user_type
			END) AS user_type
FROM ORDERS_00 m LEFT JOIN IN_user_type s
...
UPDATE ORDERS_00 AS m 
SET m.user_type = 
(SELECT s.user_type FROM ORDERS_00u AS s WHERE m.erp_id = s.erp_id)
;
...
ON COALESCE(u.pickup_zip_code,COALESCE(u.real_zip_code, u.business_zip_code)) = i.postcode
SET
    u.real_name = COALESCE(CAP_FIRST(u.real_name),COALESCE(CAP_FIRST(u.business_name), CAP_FIRST(u.pickup_name))),
    ...
    u.pickup_name = COALESCE(CAP_FIRST(u.pickup_name), CAP_FIRST(u.real_name)),
	...
	u.business_name = COALESCE(CAP_FIRST(u.business_name), CAP_FIRST(u.real_name)),
...
ALTER TABLE ORDERS_00 ADD INDEX (`pickup_city`) USING BTREE;
ALTER TABLE ORDERS_00 ADD INDEX (`real_city`) USING BTREE;
