...
SELECT DISTINCT 
    ...
    END AS CT4_product_brand,
	    CASE 	WHEN j.item_manufacturer_name = 'CIBA Vision' THEN 'Alcon'
			WHEN j.magento_sku_hu = 'ECPURE360' THEN 'eOptika Kft.'
...
DROP TABLE IF EXISTS CANCELLED_ORDERS_00f;
CREATE TABLE IF NOT EXISTS CANCELLED_ORDERS_00f
SELECT DISTINCT a.*, b.*,
        ...
        c.box_height,
        c.box_depth
FROM CANCELLED_ORDERS_00e AS a
...
ALTER TABLE CANCELLED_ORDERS_00f ADD `pack_size` FLOAT(10,2) DEFAULT NULL;
ALTER TABLE CANCELLED_ORDERS_00f ADD `package_unit` VARCHAR(32) DEFAULT NULL;
ALTER TABLE CANCELLED_ORDERS_00f ADD `lens_material` VARCHAR(64) DEFAULT NULL;
ALTER TABLE CANCELLED_ORDERS_00f ADD `product_introduction_dt` DATE DEFAULT NULL;
ALTER TABLE CANCELLED_ORDERS_00f ADD PRIMARY KEY (`sql_id`) USING BTREE;
ALTER TABLE CANCELLED_ORDERS_00f ADD INDEX `related_email_clean` (`related_email_clean`) USING BTREE;
ALTER TABLE CANCELLED_ORDERS_00f ADD INDEX `shipping_name_clean` (`shipping_name_clean`) USING BTREE;
ALTER TABLE CANCELLED_ORDERS_00f ADD INDEX `billing_zip_code` (`billing_zip_code`) USING BTREE;
ALTER TABLE CANCELLED_ORDERS_00f ADD INDEX `erp_id` (`erp_id`) USING BTREE;
ALTER TABLE CANCELLED_ORDERS_00f ADD INDEX `reference_id` (`reference_id`) USING BTREE;

INSERT INTO CANCELLED_ORDERS_00f
SELECT DISTINCT a.*, b.*,
        c.group_eng_corrected AS product_group,
        CASE WHEN c.is_spheric = 1 THEN 'Spheric'
           WHEN c.is_toric = 1 THEN 'Toric'
...
UPDATE
  ...
  INNER JOIN (
SELECT CT2_pack, MIN(created) AS product_introduction_dt FROM CANCELLED_ORDERS_00f GROUP by CT2_pack
...
DROP TABLE IF EXISTS CANCELLED_ORDERS_00f2;
CREATE TABLE IF NOT EXISTS CANCELLED_ORDERS_00f2 LIKE CANCELLED_ORDERS_00f;
INSERT INTO CANCELLED_ORDERS_00f2 SELECT * FROM CANCELLED_ORDERS_00f;