...
SELECT  a.*, 
	...
	CASE WHEN a.origin = 'orders' THEN YEAR(a.created) ELSE NULL END AS order_year,
	CASE WHEN a.origin = 'orders' THEN MONTH(a.created) ELSE NULL END AS order_month,
	CASE WHEN a.origin = 'orders' THEN DAY(a.created) ELSE NULL END AS order_day_in_month,
	CASE WHEN a.origin = 'orders' THEN weekday(a.created) ELSE NULL END AS order_weekday,
	CASE WHEN a.origin = 'orders' THEN WEEK(a.created) - WEEK(DATE_SUB(a.created, INTERVAL DAYOFMONTH(a.created)-1 DAY)) + 1 ELSE NULL END AS order_week_in_month,
	CASE WHEN a.origin = 'orders' THEN HOUR(a.created) ELSE NULL END AS order_hour,
...
SELECT DISTINCT sz.*,
				YEAR(r.created) AS order_year2,
				MONTH(r.created) AS order_month2,
				DAY(r.created) AS order_day_in_month2,
				WEEKDAY(r.created) AS order_weekday2,
				WEEK(r.created) - WEEK(DATE_SUB(r.created, INTERVAL DAYOFMONTH(r.created)-1 DAY)) + 1 AS order_week_in_month2,
				HOUR(r.created) AS order_hour2
...
SELECT DISTINCT sz.*,
				YEAR(r.created) AS order_year3,
				MONTH(r.created) AS order_month3,
				DAY(r.created) AS order_day_in_month3,
				WEEKDAY(r.created) AS order_weekday3,
				WEEK(r.created) - WEEK(DATE_SUB(r.created, INTERVAL DAYOFMONTH(r.created)-1 DAY)) + 1 AS order_week_in_month3,
				HOUR(r.created) AS order_hour3
