...
FROM (SELECT sql_id, erp_id, related_comment, item_comment, CT1_SKU, related_division FROM INVOICES_00 WHERE item_sku IN ('VOUCHER5000', 'VOUCHER10000', 'KUPON', 'KUPONKOD', 'RABAT', '0', 'SZEPSZEMEK', 'RABAT_OTHER', 'RABAT_VAT', 'RABAT-TAM') /* ez a lista a régi kedvezményeket kezeli (néhányuk group_id = 220) */) e
LEFT JOIN (SELECT * FROM IN_kupon_codes WHERE LENGTH(coupon) > 3) k
...
ALTER TABLE INVOICES_trx_coupon_codes ADD PRIMARY KEY (`sql_id`) USING BTREE;
...
UPDATE INVOICES_00 AS m
LEFT JOIN INVOICES_trx_coupon_codes AS s 
ON (m.sql_id = s.sql_id AND m.trx_coupon_code = s.trx_coupon_code AND m.product_group = s.product_group)
...
UPDATE INVOICES_00 AS m
LEFT JOIN INVOICES_trx_coupon_codes AS s 
ON (m.sql_id = s.sql_id AND m.trx_coupon_code = s.trx_coupon_code)
SET  m.coupon_code = s.trx_coupon_code
WHERE s.product_group = 'Basket'
;
