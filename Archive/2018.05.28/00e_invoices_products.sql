...
ALTER TABLE INVOICES_trx_coupon_codes ADD PRIMARY KEY (`erp_id`) USING BTREE;
...
UPDATE INVOICES_00 AS m
LEFT JOIN INVOICES_trx_coupon_codes AS s 
ON m.trx_coupon_code = s.trx_coupon_code AND m.product_group = s.product_group
...
UPDATE INVOICES_00 AS m
LEFT JOIN INVOICES_trx_coupon_codes AS s 
ON m.trx_coupon_code = s.trx_coupon_code
SET  m.coupon_code = s.trx_coupon_code
WHERE s.product_group = 'Basket'
;
