...
SELECT  a.*,
	...
	- 	CASE 	WHEN total_lens_item_number = 0 THEN COALESCE(s.shipping_cost_fix,0)/a.total_item_number /*ha nincs lencse a kosárban, akkor a tételek számával osszuk szét a fix költséget */
		...
		END
	- (COALESCE(p.payment_fee,0) + 28)/a.total_item_number
	- COALESCE(COALESCE(s.shipping_cost_weight,0)*a.item_weight_in_kg/a.total_weight_in_kg,0) /* ez a sor azért kell, mert a súly szerinti száll. ktsg-et külön számítjuk*/
    ELSE 0 END AS net_margin_wodisc_in_base_currency,
	
    CASE WHEN item_quantity > 0 THEN /*ki kell nullázni a net margin értékeket, ha stornó (jóváíró számlán szereplő) tételről van szó*/
	...
	- (COALESCE(p.payment_fee,0) + 28)/a.total_item_number
	- COALESCE(COALESCE(s.shipping_cost_weight,0)*a.item_weight_in_kg/a.total_weight_in_kg,0) /* ez a sor azért kell, mert a súly szerinti száll. ktsg-et külön számítjuk*/
	)/(a.item_net_value_in_currency*a.exchange_rate_of_currency)
	...
	END
	+
	COALESCE(COALESCE(s.shipping_cost_weight,0)*a.item_weight_in_kg/a.total_weight_in_kg,0)
	)
    ...
    CASE WHEN item_quantity > 0 THEN
    COALESCE(p.payment_fee,0)/a.total_item_number
...
FROM 
(
SELECT  m.*, n.total_item_number, n.total_lens_item_number, n.total_weight_in_kg
...
SELECT 	erp_id,
		...
		SUM(item_weight_in_kg) AS total_weight_in_kg
FROM INVOICES_00h 
