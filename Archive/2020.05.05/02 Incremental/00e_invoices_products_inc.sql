DELETE FROM `ab_cikkto_full` 
WHERE CT1_SKU = 'OCUF_BLUE_10_ML'
AND quantity_in_a_pack = 0
;
...
SET
	...
	m.bonus_rate = s.bonus_rate,
	m.private_label_product = s.private_label_product
;

ALTER TABLE INVOICES_00_inc ADD INDEX `CT2_pack` (`CT2_pack`) USING BTREE;
...
UPDATE
  INVOICES_00_inc AS C
  INNER JOIN (
SELECT CT2_pack, MIN(created) AS product_introduction_dt FROM BASE_03_TABLE GROUP by CT2_pack
) AS A ON C.CT2_pack = A.CT2_pack
SET C.product_introduction_dt = A.product_introduction_dt
;


DROP TABLE IF EXISTS INVOICES_002_inc;
CREATE TABLE IF NOT EXISTS INVOICES_002_inc LIKE INVOICES_00_inc;
INSERT INTO INVOICES_002_inc SELECT * FROM INVOICES_00_inc;
...
FROM (
		SELECT erp_id, related_comment, item_comment, CT1_SKU, related_division 
