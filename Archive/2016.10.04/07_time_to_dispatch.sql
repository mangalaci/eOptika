...
SELECT sql_id, erp_id, shipping_method, payment_method, shipping_country, CT1_SKU, CT1_SKU_name, CT2_pack, processed, connected_order_erp_id
FROM BASE_06_TABLE 
...
SELECT sql_id, erp_id, created, item_sku
...
SET @sql = CONCAT('CREATE TABLE tracking_stat SELECT reference_id, ', @sql, ' FROM sm_tracking_data GROUP BY reference_id');
