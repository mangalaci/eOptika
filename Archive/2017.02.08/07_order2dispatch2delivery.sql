DROP TABLE IF EXISTS tracking_data_ini;
CREATE TABLE tracking_data_ini
SELECT DISTINCT 
		s.*, 
...
LEFT JOIN BASE_06_TABLE AS b
ON s.reference_id = b.erp_id
ORDER BY 1
LIMIT 0
;


ALTER TABLE tracking_data_ini ADD INDEX `id` (`id`) USING BTREE;
ALTER TABLE tracking_data_ini ADD INDEX `parent_id` (`parent_id`) USING BTREE;


INSERT INTO tracking_data_ini
SELECT DISTINCT
		s.*, 
		c.track_id AS courier_track_id,
		c.date AS courier_date,
...
b.gender,
b.salutation
FROM `tracking_data_ini` a, `tracking_data_ini` b
...
INSERT INTO tracking_data_consolidator
SELECT DISTINCT b.id,
b.reference_id,
b.backup_reference_id,
b.webshop_id,
...
/*
r.processed = azért kell a proessed mezőt használni a created helyett, mert ha a rendelés nem a webről jött,
