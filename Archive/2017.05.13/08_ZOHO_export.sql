...
DROP TABLE IF EXISTS single_lens_type;
CREATE TABLE IF NOT EXISTS single_lens_type
SELECT 	user_id,
/* ha több, mint 4-féle dioptriát használ (1 vagy 2 szemre ), akkor biztosan multi-user */
	CASE WHEN COUNT(DISTINCT lens_type) = 1 THEN
		CASE WHEN COUNT(DISTINCT lens_pwr) > 4 THEN 1 ELSE 0
        END
	END	AS multi_user
FROM BASE_07_TABLE
WHERE origin = 'invoices'
AND product_group  = 'Contact lenses'
AND created BETWEEN DATE_SUB(NOW(), INTERVAL 360 DAY) AND NOW()
GROUP BY user_id
;

ALTER TABLE single_lens_type ADD PRIMARY KEY (user_id);


DROP TABLE IF EXISTS multiple_lens_type;
CREATE TABLE IF NOT EXISTS multiple_lens_type
SELECT v.user_id,
/* ha több, mint 3-féle dioptriát használ (1 vagy 2 szemre ), akkor is lehet single-user, ha volt újraillesztése */
	CASE WHEN MAX(ABS(avg_pwr_per_item))-MIN(ABS(avg_pwr_per_item)) > 0.25 THEN 1 ELSE 0
	END AS multi_user
FROM
(
SELECT 	user_id,
    lens_type,
	SUM(lens_pwr+IFNULL(lens_cyl,0))/COUNT(*) AS avg_pwr_per_item
FROM BASE_07_TABLE
WHERE origin = 'invoices'
AND product_group  = 'Contact lenses'
AND created BETWEEN DATE_SUB(NOW(), INTERVAL 360 DAY) AND NOW()
GROUP BY user_id, lens_type
) v
GROUP BY user_id
;

ALTER TABLE multiple_lens_type ADD PRIMARY KEY (user_id);


DROP TABLE IF EXISTS multiple_lens_user;
CREATE TABLE IF NOT EXISTS multiple_lens_user
SELECT s.user_id, IFNULL(s.multi_user,0) + m.multi_user AS lens_multi_user
FROM single_lens_type s
LEFT JOIN multiple_lens_type m
ON s.user_id = m.user_id
;

ALTER TABLE multiple_lens_user ADD PRIMARY KEY (user_id);

ALTER TABLE BASE_07_TABLE ADD lens_multi_user INT(2);


UPDATE
BASE_07_TABLE AS b
LEFT JOIN multiple_lens_user AS m
ON (b.user_id = m.user_id)
SET
b.lens_multi_user = m.lens_multi_user
;
