...
CREATE TABLE IF NOT EXISTS items2 LIKE items;
...
UPDATE items2 SET name_hu = REPLACE(name_hu,'Alapértelmezett', '') WHERE LOWER(name_hu) LIKE '%alapértelmezett%';


DELETE FROM items2 WHERE sku = 'AOAM.86|0000H';

/* DEDUPLICATION: egy name_hu-hoz több sku tartozik: BEGIN */
DROP TABLE IF EXISTS a1;
CREATE TABLE a1
SELECT DISTINCT i.sku
FROM items2 i, 
(
SELECT name_hu, MAX(LENGTH(sku)) AS max_length_sku
FROM items2
GROUP BY name_hu
HAVING count(DISTINCT sku) > 1
)
 a
WHERE i.name_hu = a.name_hu
AND LENGTH(i.sku) = max_length_sku
AND i.type = 'T'
;

ALTER TABLE a1 ADD PRIMARY KEY (`sku`) USING BTREE;


DELETE FROM items2 WHERE sku IN
(
SELECT sku
FROM a1
)
...
/* DEDUPLICATION: egy name_hu-hoz több sku tartozik: END */
...
SELECT DISTINCT 
    ...
    CASE 	WHEN j.parent_id IN (81, 332, 357) THEN i.sku
			WHEN j.general_name IS NOT NULL THEN j.general_name
			ELSE 'Unidentified'
	END AS CT2_pack,
	
    CASE 	WHEN j.parent_id IN (81, 332, 357) AND LENGTH(i.sku) - LENGTH(REPLACE(i.sku, '_', '')) > 1 THEN SUBSTRING_INDEX(SUBSTRING_INDEX(i.sku, '_', 2), ' ', -1)
			WHEN j.parent_id IN (81, 332, 357) AND LENGTH(i.sku) - LENGTH(REPLACE(i.sku, '_', '')) = 1 THEN SUBSTRING_INDEX(SUBSTRING_INDEX(i.sku, '_', 1), ' ', -1)
			WHEN j.parent_id IN (81, 332, 357) AND LENGTH(i.sku) - LENGTH(REPLACE(i.sku, '-', '')) > 0 THEN SUBSTRING_INDEX(SUBSTRING_INDEX(i.sku, '-', 1), ' ', -1)
