...
2. tökéletes ország egyezés, utána a megadott város benne van a városlistában: Nyiregy -> Níiregyháza
3. tökéletes ország egyezés, utána a megadott városban benne van a városlista egyik eleme: Alsónémedi É-i V.ter. -> Alsónémedi
...
shipping_zip_code,
billing_zip_code,
LOWER(shipping_city_standardized) AS shipping_city_standardized,
LOWER(billing_city_standardized) AS billing_city_standardized,
...
CASE
	...
	WHEN billing_name_flg = 'real' THEN IF(billing_address = '', shipping_address, billing_address) /* azért kell a COALESCE, hogy ha üres a billing_address */
END AS real_address,
...
CASE 
	...
	WHEN billing_name_flg = 'billing' THEN IF(billing_address = '', shipping_address, billing_address) /* azért kell a COALESCE, hogy ha üres a billing_address */ 
...
CASE 
	WHEN shipping_name_flg = 'billing' THEN IF(shipping_city_standardized = '', billing_city_standardized, shipping_city_standardized) /* azért kell a COALESCE, hogy ha üres a shipping_city */
	WHEN billing_name_flg = 'billing' THEN IF(billing_city_standardized = '', shipping_city_standardized, billing_city_standardized) /* azért kell a COALESCE, hogy ha üres a billing_city */ 
...
SET
    ...
    u.real_city_trim = special_char_replace(COALESCE(r.real_city,COALESCE(r.business_city, r.pickup_city))),
	...
	u.pickup_city_trim = special_char_replace(COALESCE(r.pickup_city,COALESCE(r.real_city, r.business_city))),
	...
	u.business_city_trim = special_char_replace(COALESCE(r.business_city, r.real_city)),
