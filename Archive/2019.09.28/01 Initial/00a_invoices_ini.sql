...
ALTER TABLE INVOICES_00 ADD wear_days SMALLINT(4);
...
update DWH_RUN_PARAMS
   set last_max_sql_id = (select max(sql_id) from INVOICES_00), -- azért kell, mert a last_max_szfej_id-t nem szabad beállítani addig, amíg az első update meg nem történt. Azért a biz_darabszam-ot szummázzuk, mert ez a legutolsó mező, amit az initial feltölt
