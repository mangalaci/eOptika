...
FROM BASE_03_table n
group by n.erp_invoice_id
;


/*div+webshop+month*/
UPDATE AGGR_ORDER as n
...
UPDATE AGGR_ORDER as n
        INNER JOIN overhead_costs_by_month_div_in_base_currency AS m
			ON n.related_division = m.related_division 
			AND n.invoice_yearmonth = m.invoice_yearmonth
		INNER JOIN revenues_wdisc_by_month_div AS r
			ON n.related_division = r.related_division 
...
UPDATE AGGR_ORDER as n
		...
		INNER JOIN all_revenues_wdisc_by_month AS r
			ON n.invoice_yearmonth = r.invoice_yearmonth	
SET n.overhead_cost_in_base_currency = round(n.overhead_cost_in_base_currency + m.overhead_cost_in_base_currency*n.revenues_wdisc_in_base_currency/r.sum_revenues_wdisc_in_base_currency)
;



/*WEBSHOPOS rendelés kiszállítással*/
UPDATE AGGR_ORDER as sz
        INNER JOIN
    incoming_orders as r ON (SUBSTR(sz.connected_order_erp_id,1,11) = r.erp_id)
SET 
...
UPDATE AGGR_ORDER m
        INNER JOIN incoming_orders as r 
...
WHERE r.related_webshop = 'ClearVis.io'	
;
...
/* AGGR_ORDER visszatöltése a BASE_03_table táblába: START */



UPDATE
BASE_03_table AS u
INNER JOIN AGGR_ORDER r
