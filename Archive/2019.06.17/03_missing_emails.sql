...
SELECT real_name, billing_zip_code, related_division FROM `BASE_00i_TABLE`
LIMIT 0;
ALTER TABLE USER_33_missing_emails ADD INDEX `real_name` (`real_name`) USING BTREE;
...
SELECT DISTINCT real_name, billing_zip_code, related_division
FROM `BASE_00i_TABLE`
WHERE LENGTH(related_email_clean) <= 3 /* nincs email, se más azonosító */
AND  LENGTH(real_name) > 3 /* viszont van név */
;

/*minden real_name és billing_zip_code kulcshoz tartozó related_email_clean*/
DROP TABLE IF EXISTS USER_34_name_zip_emails;
CREATE TABLE IF NOT EXISTS USER_34_name_zip_emails AS
SELECT real_name, billing_zip_code, related_email_clean, related_division FROM `BASE_00i_TABLE`
LIMIT 0;

ALTER TABLE USER_34_name_zip_emails ADD INDEX `related_email_clean` (`related_email_clean`) USING BTREE;
ALTER TABLE USER_34_name_zip_emails ADD INDEX `real_name` (`real_name`) USING BTREE;
...
SELECT DISTINCT real_name, billing_zip_code, related_email_clean, related_division
FROM `BASE_00i_TABLE`
WHERE  LENGTH(related_email_clean) > 3 /* van email */
AND  LENGTH(real_name) > 3 /* van név */
...
SELECT b.real_name, b.billing_zip_code, e.related_email_clean, b.related_division
...
ALTER TABLE USER_35_match_missing_emails ADD INDEX `real_name` (`real_name`) USING BTREE;
...
SELECT b.real_name, b.billing_zip_code, b.related_email_clean, b.related_division, e.crm_id
...
ALTER TABLE USER_37_matching_emails_plus_ID ADD INDEX `real_name` (`real_name`) USING BTREE;
...
SELECT DISTINCT real_name, billing_zip_code, related_division, user_id
FROM USER_38_non_matching_emails
LIMIT 0;


ALTER TABLE USER_39_non_matching_emails ADD PRIMARY KEY (`user_id`) USING BTREE;
ALTER TABLE USER_39_non_matching_emails ADD INDEX `real_name` (`real_name`) USING BTREE;
...
SELECT DISTINCT real_name, billing_zip_code, related_division, (user_id*5) AS user_id
FROM USER_38_non_matching_emails;