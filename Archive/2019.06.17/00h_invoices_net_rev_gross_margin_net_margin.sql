...
SELECT
	...
	ROUND(SUM(item_net_value_in_currency*(1+item_vat_rate/100)*exchange_rate_of_currency),2) AS gross_order_value_in_base_currency,
	ROUND(SUM(item_net_value_in_currency*(1+item_vat_rate/100)),2) AS gross_order_value_in_local_currency,
	ROUND(SUM(ABS(item_net_value_in_currency)*exchange_rate_of_currency),2) AS net_order_value
FROM INVOICES_00
GROUP BY erp_id
;
...
(  	CASE
  		WHEN n.shipping_country_standardized = s.DESTINATIONS
  			THEN
			CASE
				WHEN (s.shipping_type = 'Sprinter' AND s.DESTINATIONS = 'HUNGARY') THEN
				CASE 	WHEN COALESCE(n.pickup_city,n.real_city) = 'Budapest'
			...
			END
  		ELSE n.shipping_country_standardized =  'Rest of World (b)'
 	END
