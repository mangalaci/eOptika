...
SELECT 	DISTINCT 
		...
		billing_name,
		related_webshop,
		SUBSTRING_INDEX(SUBSTRING_INDEX(c.real_name, ' ', 1), ' ', -1) AS parse_name_1,
		SUBSTRING_INDEX(SUBSTRING_INDEX(c.real_name, ' ', 2), ' ', -1) AS parse_name_2,
		SUBSTRING_INDEX(SUBSTRING_INDEX(c.real_name, ' ', 3), ' ', -1) AS parse_name_3,	
		SUBSTRING_INDEX(SUBSTRING_INDEX(c.real_name, ' ', 4), ' ', -1) AS parse_name_4		
FROM INVOICES_00 c
;
...
CONCAT(UCASE(LEFT(c.name_4, 1)), LCASE(SUBSTRING(c.name_4, 2))) AS upper_name_4,
 		...
 		MIN(CASE
			WHEN LOWER(c.real_name) LIKE '%kornélia%' THEN 'Female'
			WHEN LOWER(c.real_name) LIKE '%kornél%' THEN 'Male'
			WHEN LOWER(SUBSTRING_INDEX(SUBSTRING_INDEX(c.real_name, ' ', 1), ' ', -1)) LIKE '%né' THEN 'Female'
			WHEN LOWER(SUBSTRING_INDEX(SUBSTRING_INDEX(c.real_name, ' ', 2), ' ', -1)) LIKE '%né' THEN 'Female'
			WHEN LOWER(SUBSTRING_INDEX(SUBSTRING_INDEX(c.real_name, ' ', 3), ' ', -1)) LIKE '%né' THEN 'Female'
...
ALTER TABLE INVOICES_00d3 ADD INDEX `real_name` (`real_name`) USING BTREE;
...
ALTER TABLE INVOICES_00d4 ADD INDEX `real_name` (`real_name`) USING BTREE;
...
ALTER TABLE INVOICES_00d5 ADD INDEX `real_name` (`real_name`) USING BTREE;
