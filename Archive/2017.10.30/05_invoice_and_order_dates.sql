...
SELECT  
    ...
    TIMESTAMPDIFF(MONTH, STR_TO_DATE(REPLACE(CONCAT(b.date_of_first_purchase,'-28'), '_', '-'), '%Y-%m-%d'), STR_TO_DATE(b.date_of_last_purchase, '%Y-%m-%d')) AS cohort_month_since,
...
ALTER TABLE order_invoice_dates ADD INDEX (`item_id`) USING BTREE;
ALTER TABLE order_invoice_dates ADD INDEX `erp_invoice_id` (`erp_invoice_id`) USING BTREE;
...
SELECT  
	...
	CASE WHEN a.origin = 'invoices' THEN
		TIMESTAMPDIFF(MONTH, STR_TO_DATE(REPLACE(CONCAT(b.date_of_first_purchase,'-28'), '_', '-'), '%Y-%m-%d'), STR_TO_DATE(REPLACE(CONCAT(b.date_of_last_purchase,'-28'), '_', '-'), '%Y-%m-%d')) 
