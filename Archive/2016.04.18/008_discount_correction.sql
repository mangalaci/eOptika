...
DROP TABLE IF EXISTS BASE_008_TABLE;
CREATE TABLE BASE_008_TABLE
SELECT x.*,
        y.rabat_sql_id
FROM BASE_007_TABLE x
 LEFT JOIN
(
SELECT
		DISTINCT b.*,
...
ALTER TABLE BASE_008_TABLE ADD PRIMARY KEY (`sql_id`) USING BTREE;
ALTER TABLE BASE_008_TABLE ADD INDEX `related_email_clean` (`related_email_clean`) USING BTREE;
ALTER TABLE BASE_008_TABLE ADD INDEX `shipping_name_clean` (`shipping_name_clean`) USING BTREE;
ALTER TABLE BASE_008_TABLE ADD INDEX `billing_zip_code` (`billing_zip_code`) USING BTREE;
