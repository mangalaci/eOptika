DROP TABLE IF EXISTS BASE_003_TABLE;
CREATE TABLE BASE_003_TABLE AS
SELECT
		 *,
...
ALTER TABLE BASE_003_TABLE
  DROP COLUMN shipping_name_trim,
  DROP COLUMN shipping_method,
  DROP COLUMN billing_method
;

ALTER TABLE BASE_003_TABLE CHANGE `shipping_name_clean` `shipping_name_clean` VARCHAR(100);  
ALTER TABLE BASE_003_TABLE CHANGE `shipping_method2` `shipping_method` VARCHAR(100);

  
ALTER TABLE BASE_003_TABLE ADD PRIMARY KEY (`sql_id`) USING BTREE;
ALTER TABLE BASE_003_TABLE ADD INDEX `related_email_clean` (`related_email_clean`) USING BTREE;
ALTER TABLE BASE_003_TABLE ADD INDEX `shipping_name_clean` (`shipping_name_clean`) USING BTREE;
ALTER TABLE BASE_003_TABLE ADD INDEX `billing_zip_code` (`billing_zip_code`) USING BTREE;
ALTER TABLE BASE_003_TABLE ADD INDEX `billing_city_clean` (`billing_city_clean`) USING BTREE;