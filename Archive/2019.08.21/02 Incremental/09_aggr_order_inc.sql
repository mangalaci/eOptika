DROP TABLE IF EXISTS AGGR_ORDER_INI;
CREATE TABLE AGGR_ORDER_INI
SELECT
		origin,
		primary_email,
		secondary_email,
		...
		END AS personal_geogr_region,
			1 AS new_entry
FROM BASE_03_TABLE
WHERE (created > (SELECT max(created) FROM INVOICES_002) AND origin = 'invoices') OR (created > (SELECT max(created) FROM ORDERS_002) AND origin = 'orders')
GROUP BY erp_invoice_id
;


/*le kell nullázni a new_entry flaget, mielőtt az új tételeket insertáljuk*/
UPDATE AGGR_ORDER
SET new_entry = 0
WHERE new_entry = 1
;
...
/*a napi új tételeket hozzárakjuk a többi, historikusan összegyűjtött tételhez*/
