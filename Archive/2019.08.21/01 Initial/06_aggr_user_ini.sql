drop table if exists AGGR_USER_UNSANITIZED_teszt;
create table AGGR_USER_UNSANITIZED_teszt (
	...
	one_before_last_purchase datetime,
	...
	revenues_wdisc_2015_2016 decimal(14),
	...
	contact_lens_user int(1),
	solution_user int(1),
	eye_drops_user int(1),
	sunglass_user int(1),
	vitamin_user int(1),
	frames_user int(1),
	lenses_for_spectacles_user int(1),
	contact_lens_trials_user int(1),
	spectacles_user int(1),
	other_product_user int(1),
	...
	date_lenses_run_out date,
	date_lens_cleaners_run_out datetime,
	contact_lens_last_purchase datetime,
	...
	brand_switcher_same_manufacturer int(1),
	brand_and_manufacturer_switcher int(1),
	repeat_prediction varchar(55),
	active_prediction varchar(55),
	repeat_prediction_date date,
	active_prediction_date date,
...
insert into AGGR_USER_UNSANITIZED_teszt
select
		t.user_id,
		max(case 	when contact_lens_user = 0 AND solution_user = 0 AND eye_drops_user = 0	THEN 'buy_me_once'
					when product_group = 'Contact lenses' 				THEN CASE WHEN DATEDIFF(CURDATE(), last_modified_date) > wear_days*item_quantity THEN 'inactive' ELSE 'active' END
					when product_group = 'Contact lens cleaners' 		THEN CASE WHEN DATEDIFF(CURDATE(), last_modified_date) > 180 THEN 'inactive' ELSE 'active' END
					when product_group = 'Eye drops' 					THEN CASE WHEN DATEDIFF(CURDATE(), last_modified_date) > 180 THEN 'inactive' ELSE 'active' END
					when product_group = 'Shipping fees' 				THEN 'inactive'
					else 'inactive'
		...
		0 brand_switcher_same_manufacturer,
		0 brand_and_manufacturer_switcher,
...
UPDATE
AGGR_USER_UNSANITIZED_teszt AS u
LEFT JOIN COHORT_01_first_last_purchase r
...
UPDATE
AGGR_USER_UNSANITIZED_teszt AS c
...
c.contact_lens_last_purchase = l.contact_lens_last_purchase
;
...
UPDATE
BASE_03_TABLE AS u
INNER JOIN AGGR_USER_UNSANITIZED_teszt r
ON r.user_id = u.user_id
SET u.one_before_last_purchase = r.one_before_last_purchase,
...
FROM (SELECT * FROM BASE_03_TABLE WHERE origin = 'invoices' ORDER BY created DESC /*azért kell csökkenő sorrendben, mert ha a user_id alatt több név is van, akkor az utolsót vegyük*/) t
GROUP BY t.user_id
ORDER BY t.created
) r
ON r.user_id = u.user_id
SET u.first_year_contact_lens_overuse_ratio = r.first_year_contact_lens_overuse_ratio
;
...
SET u.brand_switcher_same_manufacturer = s.brand_switcher_same_manufacturer,
	u.brand_and_manufacturer_switcher = s.brand_and_manufacturer_switcher
;
...
UPDATE AGGR_USER_UNSANITIZED_teszt SET primary_email = 'email_blacklist' WHERE primary_email = 'evagorda@gmail.com';
UPDATE AGGR_USER_UNSANITIZED_teszt SET secondary_email = 'email_blacklist' WHERE secondary_email = 'evagorda@gmail.com';
...
SELECT *
FROM AGGR_USER_UNSANITIZED_teszt
;
