...
DELIMITER //
CREATE PROCEDURE CityUpdate(table_name VARCHAR(64), address_type VARCHAR(32))
BEGIN
...
WHERE e.billing_city_standardized IS NOT NULL');
  ...
  DELETE FROM shipping_city_nonmatch
WHERE shipping_city_standardized IS NOT NULL
;

  DELETE FROM billing_city_nonmatch
WHERE billing_city_standardized IS NOT NULL
;
