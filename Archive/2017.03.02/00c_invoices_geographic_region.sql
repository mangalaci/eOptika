...
CASE 	WHEN shipping_name REGEXP 'Egészségpénztár|Egészség-|/TOF|PPP|/ PM|/EP'
		...
		THEN 1 ELSE 0
END AS shipping_name_pickup_flg,
...
CASE 	WHEN billing_name REGEXP 'Egészségpénztár|Egészség-|/TOF|PPP|/ PM|/EP'
		...
		THEN 1 ELSE 0
END AS billing_name_pickup_flg,

CASE 	WHEN LOWER(shipping_name) REGEXP 'bt.|kft|rt.|zrt|nyrt'
		THEN 1 ELSE 0
END AS shipping_name_billing_flg,

CASE 	WHEN LOWER(billing_name) REGEXP 'bt.|kft|rt.|zrt|nyrt'
		THEN 1 ELSE 0
END AS billing_name_billing_flg,
...
ALTER TABLE INVOICES_00c3 ADD INDEX `shipping_name` (`shipping_name`) USING BTREE;
