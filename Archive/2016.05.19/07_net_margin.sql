...
FROM BASE_06_TABLE m LEFT JOIN
(
...
ALTER TABLE BASE_07_TABLE CHANGE `related_webshop` `related_webshop` VARCHAR(20) COMMENT 'Webshop the order coming from: LenteContatto.it, b2b, eMAG.hu, eOptika.hu, lealkudtuk.hu, napszemuvegcenter.hu, napszemuvegplaza.hu, netoptika.ro, netoptika.sk, policenapszemuveg.hu, vatera.hu. Not every item has a value.';
...
ALTER TABLE BASE_07_TABLE CHANGE `item_weight_in_kg` `item_weight_in_kg` FLOAT COMMENT 'Weight of the item * Quantity';
...
ALTER TABLE BASE_07_TABLE CHANGE `reminder_day` `reminder_day` DATE COMMENT 'Az a nap, amikor az emlékeztetőt kapta';
...
ALTER TABLE BASE_07_TABLE CHANGE `wear_days` `wear_days` VARCHAR(20) COMMENT 'Mennyi ideig használható a csomag?';
ALTER TABLE BASE_07_TABLE CHANGE `wear_duration` `wear_duration` VARCHAR(20) COMMENT '';
...
ALTER TABLE BASE_07_TABLE CHANGE `invoice_hour` `invoice_hour` TIME COMMENT 'Hour of invoicing ( 1 to 24, number, rounded down)';
...
ALTER TABLE BASE_07_TABLE CHANGE `order_hour` `order_hour` TIME COMMENT 'Hour of order received ( 1 to 24, number, rounded down)';
...
ALTER TABLE BASE_07_TABLE CHANGE `cohort_month_since` `cohort_month_since` INT(6) COMMENT 'Hanyadik honapban tortent a tranzakcio a user elso vasarlasa ota (amikor bekerult a cohortba)';
ALTER TABLE BASE_07_TABLE CHANGE `user_cum_transactions` `user_cum_transactions` FLOAT COMMENT 'User hanyadik tranzakcioja';
...
ALTER TABLE BASE_07_TABLE CHANGE `related_webshop` `related_webshop` VARCHAR(50) COMMENT 'Webshop the order coming from: LenteContatto.it, b2b, eMAG.hu, eOptika.hu, lealkudtuk.hu, napszemuvegcenter.hu, napszemuvegplaza.hu, netoptika.ro, netoptika.sk, policenapszemuveg.hu, vatera.hu. Not every item has a value., From Adams incoming_order table';
