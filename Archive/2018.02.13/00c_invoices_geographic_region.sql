...
UPDATE INVOICES_00 AS m
        LEFT JOIN
    INVOICES_00c1 AS s ON m.sql_id = s.sql_id
SET
...
SELECT 	DISTINCT u.sql_id,
		...
		r.business_zip_code,
		/*le kell szedni az ékezetes betűket a település nevéről, mert sok az elírás*/
		REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(r.business_city,'á','a'),'é','e'),'í','i'),'ó','o'),'ú','u'),'ő','ö'),'ű','ü') AS business_city_trim,
		r.pickup_name,
		r.pickup_address,
		r.pickup_zip_code,
		/*le kell szedni az ékezetes betűket a település nevéről, mert sok az elírás*/
		REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(r.pickup_city,'á','a'),'é','e'),'í','i'),'ó','o'),'ú','u'),'ő','ö'),'ű','ü') AS pickup_city_trim,
...
SELECT DISTINCT telepules,
        MAX(megye) AS megye,
        MAX(meret) AS meret,
REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(telepules,'Ő','O'),'Ö','O'),'Ú','U'),'É','E'),'Á','A'),'á','a'),'é','e'),'í','i'),'ó','o'),'ú','u'),'ő','o'),'ű','u'),'ö','o') AS telepules_clean
FROM IN_telepules_megye
GROUP BY telepules;
