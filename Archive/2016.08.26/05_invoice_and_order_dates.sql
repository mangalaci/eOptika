...
ALTER TABLE BASE_05_TABLE ADD INDEX `origin` (`origin`) USING BTREE;

INSERT INTO BASE_05_TABLE
SELECT  a.*, 
    b.date_of_first_purchase AS cohort_id,
    CASE WHEN a.origin = 'invoices' THEN CONCAT(YEAR(a.created),'_',lpad(MONTH(created),2,'0')) ELSE NULL END AS invoice_yearmonth,
	...
	CASE WHEN a.origin = 'orders' THEN WEEK(a.created) - WEEK(DATE_SUB(created, INTERVAL DAYOFMONTH(created)-1 DAY)) + 1 ELSE NULL END AS order_week_in_month, 
	CASE WHEN a.origin = 'orders' THEN HOUR(a.created) ELSE NULL END AS order_hour,
	CASE WHEN a.origin = 'invoices' THEN 
		TIMESTAMPDIFF(MONTH, STR_TO_DATE(REPLACE(CONCAT(b.date_of_first_purchase,'-28'), '_', '-'), '%Y-%m-%d'), STR_TO_DATE(created, '%Y-%m-%d')) 
