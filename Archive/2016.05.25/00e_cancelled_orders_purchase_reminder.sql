DROP TABLE IF EXISTS CANCELLED_ORDERS_00e;
CREATE TABLE IF NOT EXISTS CANCELLED_ORDERS_00e LIKE CANCELLED_ORDERS_00d;
ALTER TABLE `CANCELLED_ORDERS_00e` ADD `reminder_day` VARCHAR(10) NOT NULL;

INSERT INTO CANCELLED_ORDERS_00e
SELECT DISTINCT a.*, CASE WHEN max(nap) IS NOT NULL THEN max(nap) ELSE '9999-12-31' END reminder_day
FROM CANCELLED_ORDERS_00d AS a LEFT JOIN IN_vasarlas_emlekezteto AS b
ON a.related_email_clean = b.email
AND left(a.item_sku,12) = left(b.cikkek,12)
AND DATE(nap) < DATE(created)
GROUP BY a.sql_id;

ALTER TABLE CANCELLED_ORDERS_00e ADD INDEX `item_sku` (`item_sku`) USING BTREE;