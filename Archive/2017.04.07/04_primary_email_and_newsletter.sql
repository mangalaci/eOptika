...
DROP TABLE IF EXISTS BASE_04a_TABLE_aux;
CREATE TABLE IF NOT EXISTS BASE_04a_TABLE_aux
  SELECT DISTINCT a.user_id,
  IF(b.primary_email <> '', b.primary_email, a.related_email_clean) AS primary_email,
  b.secondary_email
...
ALTER TABLE BASE_04a_TABLE_aux ADD INDEX (`user_id`) USING BTREE;


DROP TABLE IF EXISTS BASE_04a_TABLE;
CREATE TABLE IF NOT EXISTS BASE_04a_TABLE
  SELECT DISTINCT a.*,
/* keresztnev.csaladnev@eoptikafiktiv.hu email cím hozzátétele, ha nincs email cím megadva */
  IF(b.primary_email = '', CONCAT(LOWER(TRIM(first_name)), '.', LOWER(TRIM(last_name)), '@eoptikafiktiv.hu'), b.primary_email) AS primary_email,
  b.secondary_email
  FROM `BASE_03_TABLE` AS a 
  LEFT JOIN BASE_04a_TABLE_aux AS b
  ON a.user_id = b.user_id;
...
ALTER TABLE BASE_04c_TABLE ADD INDEX (`related_email_clean`) USING BTREE;
ALTER TABLE BASE_04c_TABLE ADD INDEX (`gender`) USING BTREE;
