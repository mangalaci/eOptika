...
DROP INDEX `source_medium` ON IN_KPI_marketing_prime_channels;
...
/*VIGYÁZAT: 05-ös táblára épít*/
DROP TABLE IF EXISTS BASE_06eo_TABLE;
CREATE TABLE BASE_06eo_TABLE
...
ALTER TABLE BASE_06eo_TABLE ADD PRIMARY KEY(`sql_id`);
ALTER TABLE BASE_06eo_TABLE ADD INDEX `new_reference_id` (`new_reference_id`) USING BTREE;
ALTER TABLE BASE_06eo_TABLE CHANGE `new_reference_id` `new_reference_id` INT(10) NULL DEFAULT NULL;



DROP TABLE IF EXISTS BASE_06m_TABLE;
CREATE TABLE BASE_06m_TABLE
SELECT b.*, channel AS trx_marketing_channel
FROM BASE_06eo_TABLE AS b LEFT JOIN IN_prime_channels AS e
ON b.new_reference_id = e.orderid
LIMIT 0;



INSERT INTO BASE_06m_TABLE
SELECT b.*, channel AS trx_marketing_channel
FROM BASE_06eo_TABLE AS b LEFT JOIN IN_prime_channels AS e
ON b.new_reference_id = e.orderid
;
...
ALTER TABLE BASE_06m_TABLE ADD PRIMARY KEY(`sql_id`);
...
FROM BASE_05_TABLE AS b LEFT JOIN BASE_06m_TABLE AS e
