...
ALTER TABLE BASE_00d_TABLE ADD INDEX `billing_country` (`billing_country`) USING BTREE;
ALTER TABLE BASE_00d_TABLE ADD INDEX `shipping_country` (`shipping_country`) USING BTREE;
ALTER TABLE BASE_00d_TABLE ADD INDEX `related_division` (`related_division`) USING BTREE;
ALTER TABLE BASE_00d_TABLE ADD INDEX `shipping_name` (`shipping_name`) USING BTREE;
ALTER TABLE BASE_00d_TABLE ADD INDEX `billing_name` (`billing_name`) USING BTREE;




INSERT INTO BASE_00d_TABLE
SELECT a.*, gender
FROM BASE_00c3_TABLE AS a LEFT JOIN
(SELECT a.billing_name, a.shipping_name_clean, MAX(b.nev) AS keresztnev, MAX(b.nem) AS gender
  FROM BASE_00c3_TABLE AS a LEFT JOIN IN_gender AS b ON 
