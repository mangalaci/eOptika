...
SELECT  /*a kedvezmény tételek szummája*/
		...
		SUM(item_net_value_in_currency) AS rabat_item_net_value_in_currency
FROM  ORDERS_00
WHERE item_note LIKE '%Szemvizsgálat vásárlási kedvezmény%'
GROUP BY erp_id
) b,
(
SELECT  /*az eredeti tételek szummája*/
...
DELETE FROM ORDERS_00 
USING ORDERS_00, bbb
WHERE bbb.erp_id = ORDERS_00.erp_id 
AND bbb.item_sku = ORDERS_00.item_sku
;


/*2. Tétel értékkel EGYENÉRÉTKŰ kupon: */
/*2.1. A később törlendő RABAT sor megjelölése*/  
DROP TABLE IF EXISTS ORDERS_rabat;
CREATE TABLE ORDERS_rabat
SELECT
    DISTINCT b.*,
...
ALTER TABLE ORDERS_rabat ADD PRIMARY KEY (`sql_id`) USING BTREE;
...
ALTER TABLE ORDERS_00g ADD PRIMARY KEY (`sql_id`) USING BTREE;


UPDATE
ORDERS_00 AS u
LEFT JOIN ORDERS_00g r
ON r.sql_id = u.sql_id
SET
u.item_net_value_in_currency = r.item_net_value_in_currency
WHERE r.sql_id IS NOT NULL
...
DELETE FROM ORDERS_00 
USING ORDERS_00, ORDERS_rabat 
WHERE ORDERS_rabat.sql_id = ORDERS_00.sql_id
;
