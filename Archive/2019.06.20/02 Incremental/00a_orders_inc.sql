...
WHERE i.created  NOT IN (SELECT DISTINCT created FROM BASE_09_TABLE WHERE origin = 'orders') /* csak az új sorok kellenek */
...
AND i.created  NOT IN (SELECT DISTINCT created FROM BASE_09_TABLE WHERE origin = 'orders') /* csak az új sorok kellenek */
AND i.deletion_comment NOT IN ('Automatikus törlés módosítás miatt', '5) Egyéb: 2x')
AND i.is_deleted = 'no'
AND LOWER(i.deletion_comment) NOT LIKE '%dupl%'
AND LOWER(i.deletion_comment) NOT LIKE '%teszt%'
