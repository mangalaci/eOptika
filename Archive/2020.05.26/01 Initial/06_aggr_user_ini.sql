...
create table AGGR_USER_UNSANITIZED (
...
UPDATE
BASE_03_TABLE AS u
INNER JOIN AGGR_USER_UNSANITIZED r
...
SET 
	...
	u.other_product_user = r.spectacles_user
;
...
SELECT 	user_id,
		...
		wear_days,
		first_purchase
FROM BASE_03_TABLE WHERE origin = 'invoices' ORDER BY created DESC /*azért kell csökkenő sorrendben, mert ha a user_id alatt több név is van, akkor az utolsót vegyük*/
) t
GROUP BY t.user_id
;
...
DROP TABLE IF EXISTS BASE_03_TABLE_contact_lenses;
CREATE TABLE IF NOT EXISTS BASE_03_TABLE_contact_lenses
SELECT 
id,
item_id,
CT1_sku,
...
ALTER TABLE BASE_03_TABLE_contact_lenses ADD INDEX (`CT1_sku`) USING BTREE;
...
FROM BASE_03_TABLE_contact_lenses
GROUP BY CT1_sku
HAVING COUNT(*) > 6
;
...
ALTER TABLE AGGR_USER_UNSANITIZED ADD INDEX (`primary_email`) USING BTREE;
ALTER TABLE AGGR_USER_UNSANITIZED ADD INDEX (`secondary_email`) USING BTREE;
