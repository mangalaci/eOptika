DROP TABLE IF EXISTS BASE_00i_TABLE_inc;
CREATE TABLE BASE_00i_TABLE_inc
SELECT 	DISTINCT
		sz.sql_id,
		sz.erp_id,
		sz.reference_id, 
		...
		sz.personal_name,
		sz.personal_address,
		sz.personal_zip_code,
		sz.personal_city,
		sz.personal_city_size,
		sz.personal_province,
		sz.personal_country,
		sz.pickup_name,
		sz.pickup_address,
		sz.pickup_zip_code,
		...
		sz.new_entry,
		sz.bonus_rate,
		sz.private_label_product		
FROM INVOICES_00_inc sz
UNION ALL
SELECT 	DISTINCT
		r.sql_id, 
		r.erp_id, 
		...
		r.new_entry,
		NULL AS bonus_rate,
		r.private_label_product		
FROM ORDERS_00 r
;


ALTER TABLE BASE_00i_TABLE_inc ADD `id` INT UNSIGNED NOT NULL AUTO_INCREMENT FIRST, ADD PRIMARY KEY (`id`);
ALTER TABLE BASE_00i_TABLE_inc ADD INDEX (`sql_id`) USING BTREE COMMENT 'Unique ID given by the KPI system to each item';
ALTER TABLE BASE_00i_TABLE_inc ADD INDEX (`erp_id`) USING BTREE COMMENT 'Order number given by the ERP system automatically. First 2 characters:
SO - Hungary
SI - Italy
SR - Romania
SS - Slovakia
Follow by the year (11, 12, 13, etc)';
ALTER TABLE BASE_00i_TABLE_inc ADD INDEX (`buyer_email`) USING BTREE;
ALTER TABLE BASE_00i_TABLE_inc ADD INDEX (`personal_name`) USING BTREE;
ALTER TABLE BASE_00i_TABLE_inc ADD INDEX (`billing_zip_code`) USING BTREE;
ALTER TABLE BASE_00i_TABLE_inc ADD INDEX (`reference_id`) USING BTREE COMMENT 'Order number coming from the webstore automatically or in case of manual entry it is a free text cell. This goes through on all the tables.';
ALTER TABLE BASE_00i_TABLE_inc ADD INDEX (`shipping_phone`) USING BTREE;
