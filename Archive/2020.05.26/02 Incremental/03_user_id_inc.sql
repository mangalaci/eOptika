...
ALTER TABLE BASE_00i_TABLE_inc ADD last_update TIMESTAMP DEFAULT CURRENT_TIMESTAMP(0);
...
UPDATE BASE_00i_TABLE_inc AS m
	...
	ON m.personal_name = s.personal_name
SET
    m.user_id = s.user_id
WHERE m.user_id IS NULL
;
...
UPDATE BASE_00i_TABLE_inc AS m
        LEFT JOIN
    USER_39_non_matching_emails AS s 
	ON m.billing_zip_code = s.billing_zip_code
SET
    m.user_id = s.user_id
WHERE m.user_id IS NULL
;
