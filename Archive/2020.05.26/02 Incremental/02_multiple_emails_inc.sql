/*1. Ugyanahhoz a vevőhöz tartozó emailek egyesítése personal_NAME és BILLING_ZIP_CODE alapján*/
...
UPDATE USER_02_new_emails AS m
        LEFT JOIN
    USER_04_personal_name_billing_zip_code_key AS s ON (m.personal_name = s.personal_name AND m.billing_zip_code = s.billing_zip_code)
SET
    m.user_id = s.user_id,
	m.personal_name_billing_zip_code_flg = s.user_id/s.user_id
;
...
WHERE m.personal_name_billing_zip_code_flg IS NULL
;



/*3. Ugyanahhoz a vevőhöz tartozó emailek egyesítése SHIPPING_PHONE és personal_NAME alapján*/
DROP TABLE IF EXISTS USER_07_shipping_phone_personal_name_clean_key;
CREATE TABLE IF NOT EXISTS USER_07_shipping_phone_personal_name_clean_key
SELECT 	shipping_phone, 
		personal_name,
		MAX(user_id) AS user_id, /* a MAX függvény a NULL user_id mellőzése miatt kell */
		COUNT(DISTINCT buyer_email) AS num_of_emails
...
UPDATE USER_02_new_emails AS m
        LEFT JOIN
    USER_07_shipping_phone_personal_name_clean_key AS s ON (m.shipping_phone = s.shipping_phone AND m.personal_name = s.personal_name)
SET
    m.user_id = s.user_id
WHERE m.personal_name_billing_zip_code_flg IS NULL
