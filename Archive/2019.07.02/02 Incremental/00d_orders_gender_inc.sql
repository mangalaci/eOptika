...
FROM ORDERS_00 c
WHERE new_entry = 1 /*csak az uj sorokat updateeljük*/
;
...
CONCAT(UCASE(LEFT(c.parse_name_4, 1)), LCASE(SUBSTRING(c.parse_name_4, 2))) AS upper_name_4,
		...
		END) AS gender,
		MIN(CASE
			WHEN LOWER(c.real_name) LIKE '%kornélia%' THEN 0
			WHEN LOWER(c.real_name) LIKE '%kornél%' THEN 0
			WHEN LOWER(c.real_name) LIKE '%czene%' THEN 0
			WHEN LOWER(SUBSTRING_INDEX(SUBSTRING_INDEX(c.real_name, ' ', 1), ' ', -1)) LIKE '%né' THEN 1
			WHEN LOWER(SUBSTRING_INDEX(SUBSTRING_INDEX(c.real_name, ' ', 2), ' ', -1)) LIKE '%né' THEN 1
			WHEN LOWER(SUBSTRING_INDEX(SUBSTRING_INDEX(c.real_name, ' ', 3), ' ', -1)) LIKE '%né' THEN 1
			WHEN LOWER(SUBSTRING_INDEX(SUBSTRING_INDEX(c.real_name, ' ', 4), ' ', -1)) LIKE '%né' THEN 1
			ELSE 0
		END) AS ne
...
ALTER TABLE ORDERS_00d2 ADD INDEX `related_webshop` (`related_webshop`) USING BTREE;
ALTER TABLE ORDERS_00d2 ADD INDEX `gender` (`gender`) USING BTREE;
...
UPDATE ORDERS_00 AS m
        LEFT JOIN
    ORDERS_00d3 AS s ON m.sql_id = s.sql_id
SET
