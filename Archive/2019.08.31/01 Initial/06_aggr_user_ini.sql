...
create table AGGR_USER_UNSANITIZED (
	...
	one_before_last_purchase timestamp,
	...
	date_lenses_run_out timestamp,
	date_lens_cleaners_run_out timestamp,
	contact_lens_last_purchase timestamp,
	...
	repeat_prediction_date timestamp,
	active_prediction_date timestamp,
...
select
		...
		CASE WHEN product_group = 'Contact lenses' AND created BETWEEN DATE_SUB(CASE WHEN t.product_group = 'Contact lenses' THEN t.last_modified_date END, INTERVAL 360 DAY) AND CASE WHEN t.product_group = 'Contact lenses' THEN t.last_modified_date END THEN MAX(lens_pwr) END AS pwr_eye1,
