...
SELECT DISTINCT 
    ...
    CASE 	WHEN i.name_hu LIKE '%Acuvue%' THEN 'Acuvue'
       ...
       WHEN CASE WHEN j.parent_id = 0 THEN i.name_hu ELSE j.general_name END LIKE '%sole%' THEN 'Sunglasses'
       WHEN CASE WHEN j.parent_id = 0 THEN i.name_hu ELSE j.general_name END LIKE '%vista%' THEN 'Glasses'
	...
	j.sku_eoptika_hu AS CT2_sku,
					...
					CASE 	WHEN j.is_spheric = 1 THEN 'Spheric'
							WHEN j.is_toric = 1 THEN 'Toric'
							WHEN j.is_multifocal = 1 THEN 'Multifocal'
							ELSE 'Other'
					END AS lens_type,
					j.is_color,
...
ALTER TABLE ORDERS_00e ADD INDEX `related_email_clean` (`related_email_clean`) USING BTREE;
ALTER TABLE ORDERS_00e ADD INDEX `real_name_clean` (`real_name_clean`) USING BTREE;
ALTER TABLE ORDERS_00e ADD INDEX `billing_zip_code` (`billing_zip_code`) USING BTREE;
ALTER TABLE ORDERS_00e ADD INDEX `erp_id` (`erp_id`) USING BTREE;
ALTER TABLE ORDERS_00e ADD INDEX `reference_id` (`reference_id`) USING BTREE;
...
UPDATE
  ORDERS_00e AS C
  INNER JOIN (
SELECT CT2_pack, MIN(created) AS product_introduction_dt FROM ORDERS_00e GROUP by CT2_pack
) AS A ON C.CT2_pack = A.CT2_pack
SET C.product_introduction_dt = A.product_introduction_dt
;


DROP TABLE IF EXISTS ORDERS_00e2;
CREATE TABLE IF NOT EXISTS ORDERS_00e2 LIKE ORDERS_00e;
INSERT INTO ORDERS_00e2 SELECT * FROM ORDERS_00e;