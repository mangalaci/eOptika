...
DROP TABLE IF EXISTS CANCELLED_ORDERS_00d3;
CREATE TABLE CANCELLED_ORDERS_00d3
SELECT 	DISTINCT c.*,

 IF(c.name_1 ='' , '' , IF(g1.first_name IS NULL , IF(c.name_1 LIKE '%né', 'first_name','last_name'), 'first_name')) AS name_ind_1,
 IF(c.name_2 ='' , '' , IF(g2.first_name IS NULL , IF(c.name_2 LIKE '%né', 'first_name','last_name'), 'first_name')) AS name_ind_2,
 IF(c.name_3 ='' , '' , IF(g3.first_name IS NULL , IF(c.name_3 LIKE '%né', 'first_name','last_name'), 'first_name')) AS name_ind_3,
 IF(c.name_4 ='' , '' , IF(g4.first_name IS NULL , IF(c.name_4 LIKE '%né', 'first_name','last_name'), 'first_name')) AS name_ind_4,
 
CONCAT(UCASE(LEFT(c.name_1, 1)), SUBSTRING(c.name_1, 2)) AS upper_name_1,
CONCAT(UCASE(LEFT(c.name_2, 1)), SUBSTRING(c.name_2, 2)) AS upper_name_2,
CONCAT(UCASE(LEFT(c.name_3, 1)), SUBSTRING(c.name_3, 2)) AS upper_name_3,
CONCAT(UCASE(LEFT(c.name_4, 1)), SUBSTRING(c.name_4, 2)) AS upper_name_4,
...
ALTER TABLE CANCELLED_ORDERS_00d3 ADD PRIMARY KEY (`sql_id`) USING BTREE;
ALTER TABLE CANCELLED_ORDERS_00d3 ADD INDEX `shipping_name_clean` (`shipping_name_clean`) USING BTREE;
ALTER TABLE CANCELLED_ORDERS_00d3 ADD INDEX `shipping_country_standardized` (`shipping_country_standardized`) USING BTREE;
ALTER TABLE CANCELLED_ORDERS_00d3 ADD INDEX `gender` (`gender`) USING BTREE;


/*név tagok és megszólítás összetétele */
DROP TABLE IF EXISTS CANCELLED_ORDERS_00d4;
CREATE TABLE CANCELLED_ORDERS_00d4
SELECT DISTINCT a.*,
		c.prefix AS salutation,
...
FROM CANCELLED_ORDERS_00d3 AS a
LEFT JOIN IN_megszolitasi_formak c
ON (a.gender = c.gender AND a.shipping_country_standardized = c.country)
;


ALTER TABLE CANCELLED_ORDERS_00d4 ADD PRIMARY KEY (`sql_id`) USING BTREE;
ALTER TABLE CANCELLED_ORDERS_00d4 ADD INDEX `shipping_name_clean` (`shipping_name_clean`) USING BTREE;
ALTER TABLE CANCELLED_ORDERS_00d4 ADD INDEX `shipping_country_standardized` (`shipping_country_standardized`) USING BTREE;
ALTER TABLE CANCELLED_ORDERS_00d4 ADD INDEX `gender` (`gender`) USING BTREE;



DROP TABLE IF EXISTS CANCELLED_ORDERS_00d5;
CREATE TABLE CANCELLED_ORDERS_00d5
SELECT DISTINCT a.*,
		REPLACE(REPLACE(REPLACE(REPLACE(full_name_raw,'/',''),'Dr ','Dr. '),'  !','!'),' !','!') AS full_name
FROM CANCELLED_ORDERS_00d4 a
;


ALTER TABLE CANCELLED_ORDERS_00d5
...
ALTER TABLE CANCELLED_ORDERS_00d5 ADD PRIMARY KEY (`sql_id`) USING BTREE;
ALTER TABLE CANCELLED_ORDERS_00d5 ADD INDEX `shipping_name_clean` (`shipping_name_clean`) USING BTREE;
ALTER TABLE CANCELLED_ORDERS_00d5 ADD INDEX `shipping_country_standardized` (`shipping_country_standardized`) USING BTREE;
ALTER TABLE CANCELLED_ORDERS_00d5 ADD INDEX `gender` (`gender`) USING BTREE;