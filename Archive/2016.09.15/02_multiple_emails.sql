...
CREATE TABLE IF NOT EXISTS USER_03_shipping_name_billing_zip_code_key
...
INSERT INTO USER_03_shipping_name_billing_zip_code_key
SELECT shipping_name_clean, billing_zip_code, COUNT(DISTINCT related_email_clean) AS num_of_emails
FROM `BASE_00i_TABLE`
WHERE LENGTH(shipping_name_clean) > 3 /*üres nevek kizárva*/
AND LENGTH(related_email_clean) > 3 /*üres emailek kizárva*/
GROUP BY shipping_name_clean, billing_zip_code
HAVING COUNT(DISTINCT related_email_clean) > 1;
ALTER TABLE USER_03_shipping_name_billing_zip_code_key ADD user_id INTEGER NOT NULL AUTO_INCREMENT PRIMARY KEY;

DROP TABLE IF EXISTS USER_04_shipping_name_billing_zip_code_key;
CREATE TABLE IF NOT EXISTS USER_04_shipping_name_billing_zip_code_key
SELECT shipping_name_clean, billing_zip_code, user_id, 1 AS shipping_name_billing_zip_code_flg
FROM USER_03_shipping_name_billing_zip_code_key
LIMIT 0;

INSERT INTO USER_04_shipping_name_billing_zip_code_key
...
CREATE TABLE IF NOT EXISTS USER_05_shipping_name_billing_zip_code_key
...
FROM BASE_00i_TABLE AS a, USER_04_shipping_name_billing_zip_code_key AS b
...
INSERT INTO USER_05_shipping_name_billing_zip_code_key
SELECT DISTINCT a.shipping_phone, a.shipping_name_clean, a.billing_zip_code, a.related_email_clean, a.related_division, b.user_id, b.shipping_name_billing_zip_code_flg
FROM BASE_00i_TABLE AS a, USER_04_shipping_name_billing_zip_code_key AS b
WHERE a.shipping_name_clean = b.shipping_name_clean
AND a.billing_zip_code = b.billing_zip_code;
...
CREATE TABLE IF NOT EXISTS USER_06_shipping_name_billing_zip_code_key LIKE USER_05_shipping_name_billing_zip_code_key;
INSERT INTO USER_06_shipping_name_billing_zip_code_key
SELECT *
...
FROM BASE_00i_TABLE AS a, USER_14_shipping_phone_billing_zip_code_key AS b
...
CREATE TABLE IF NOT EXISTS USER_23_shipping_phone_shipping_name_clean_key
...
INSERT INTO USER_23_shipping_phone_shipping_name_clean_key
SELECT shipping_phone, shipping_name_clean, COUNT(DISTINCT related_email_clean) AS num_of_emails
FROM `BASE_00i_TABLE`
WHERE LENGTH(shipping_phone) > 7 /*nem teljes telefonszámok kizárva*/
AND shipping_phone NOT LIKE '%--%' /*hamis telefonszámok kizárva*/
...
GROUP BY shipping_phone, shipping_name_clean
HAVING COUNT(DISTINCT related_email_clean) > 1;
ALTER TABLE USER_23_shipping_phone_shipping_name_clean_key ADD user_id INTEGER NOT NULL AUTO_INCREMENT PRIMARY KEY;

DROP TABLE IF EXISTS USER_24_shipping_phone_shipping_name_clean_key;
CREATE TABLE IF NOT EXISTS USER_24_shipping_phone_shipping_name_clean_key
SELECT shipping_phone, shipping_name_clean, user_id
FROM USER_23_shipping_phone_shipping_name_clean_key
LIMIT 0;

INSERT INTO USER_24_shipping_phone_shipping_name_clean_key
...
CREATE TABLE IF NOT EXISTS USER_25_shipping_phone_shipping_name_clean_key
SELECT DISTINCT a.shipping_phone, a.shipping_name_clean, a.related_email_clean, a.related_division, b.user_id
FROM BASE_00i_TABLE AS a, USER_24_shipping_phone_shipping_name_clean_key AS b
WHERE a.shipping_phone = b.shipping_phone
...
INSERT INTO USER_25_shipping_phone_shipping_name_clean_key
SELECT DISTINCT a.shipping_phone, a.shipping_name_clean, a.related_email_clean, a.related_division, b.user_id
FROM BASE_00i_TABLE AS a, USER_24_shipping_phone_shipping_name_clean_key AS b
WHERE a.shipping_phone = b.shipping_phone
AND a.shipping_name_clean = b.shipping_name_clean;
...
CREATE TABLE IF NOT EXISTS USER_26_shipping_phone_shipping_name_clean_key
...
ALTER TABLE `USER_26_shipping_phone_shipping_name_clean_key`
	ADD INDEX `user_id` (`user_id`),
...
INSERT INTO USER_26_shipping_phone_shipping_name_clean_key
SELECT DISTINCT a.shipping_phone, a.shipping_name_clean, a.related_email_clean, a.related_division,
CASE 
	WHEN c.shipping_phone_billing_zip_code_flg = 1 THEN c.user_id /*ha már volt shipping_name + billing_zip_code egyesítés, akkor az maradjon meg*/
	WHEN b.shipping_name_billing_zip_code_flg = 1 THEN b.user_id 
