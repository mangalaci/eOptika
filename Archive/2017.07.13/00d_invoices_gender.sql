...
SELECT 	DISTINCT c.*,
		SUBSTRING_INDEX(SUBSTRING_INDEX(c.real_name_clean, ' ', 1), ' ', -1) AS parse_name_1,
		SUBSTRING_INDEX(SUBSTRING_INDEX(c.real_name_clean, ' ', 2), ' ', -1) AS parse_name_2,
		SUBSTRING_INDEX(SUBSTRING_INDEX(c.real_name_clean, ' ', 3), ' ', -1) AS parse_name_3,	
		SUBSTRING_INDEX(SUBSTRING_INDEX(c.real_name_clean, ' ', 4), ' ', -1) AS parse_name_4		
FROM INVOICES_00c8 c
;


ALTER TABLE INVOICES_00d1 ADD PRIMARY KEY (`sql_id`) USING BTREE;
ALTER TABLE INVOICES_00d1 ADD INDEX `real_name_clean` (`real_name_clean`) USING BTREE;
...
ALTER TABLE INVOICES_00d2 ADD INDEX `real_name_clean` (`real_name_clean`) USING BTREE;
ALTER TABLE INVOICES_00d2 ADD INDEX `shipping_country_standardized` (`shipping_country_standardized`) USING BTREE;
ALTER TABLE INVOICES_00d2 ADD INDEX `real_name` (`real_name`) USING BTREE;
...
CONCAT(UCASE(LEFT(c.name_4, 1)), LCASE(SUBSTRING(c.name_4, 2))) AS upper_name_4,
 		...
 		MIN(CASE
			WHEN LOWER(c.real_name_clean) LIKE '%kornélia%' THEN 'Female'
			WHEN LOWER(c.real_name_clean) LIKE '%kornél%' THEN 'Male'
			WHEN LOWER(SUBSTRING_INDEX(SUBSTRING_INDEX(c.real_name_clean, ' ', 1), ' ', -1)) LIKE '%né' THEN 'Female'
			WHEN LOWER(SUBSTRING_INDEX(SUBSTRING_INDEX(c.real_name_clean, ' ', 2), ' ', -1)) LIKE '%né' THEN 'Female'
			WHEN LOWER(SUBSTRING_INDEX(SUBSTRING_INDEX(c.real_name_clean, ' ', 3), ' ', -1)) LIKE '%né' THEN 'Female'
...
ALTER TABLE INVOICES_00d3 ADD INDEX `real_name_clean` (`real_name_clean`) USING BTREE;
ALTER TABLE INVOICES_00d3 ADD INDEX `shipping_country_standardized` (`shipping_country_standardized`) USING BTREE;
ALTER TABLE INVOICES_00d3 ADD INDEX `related_webshop` (`related_webshop`) USING BTREE;
ALTER TABLE INVOICES_00d3 ADD INDEX `gender` (`gender`) USING BTREE;
...
DROP TABLE IF EXISTS INVOICES_00d4;
CREATE TABLE INVOICES_00d4
SELECT DISTINCT a.*,
		CASE 
			WHEN c.webshop = '' AND shipping_country_standardized = 'Hungary' THEN 'Kedves'
			WHEN c.webshop = '' AND shipping_country_standardized <> 'Hungary' THEN 'Dear'
			ELSE c.prefix
		END AS salutation,
		CASE WHEN shipping_country_standardized = 'Hungary' THEN
			CASE 
				WHEN name_ind_1 <> 'first_name' AND name_ind_2 <> 'first_name' AND name_ind_3 <> 'first_name' AND name_ind_4 <> 'first_name' 
					THEN CONCAT(real_name) /*amikor semmilyen keresztnév nincs a névben: céges név*/
		...
		CASE WHEN shipping_country_standardized = 'Hungary' THEN
			...
			WHEN name_ind_1 <> 'first_name' AND name_ind_2 <> 'first_name' AND name_ind_3 <> 'first_name' AND name_ind_4 <> 'first_name' THEN
						CONCAT(real_name) /*amikor semmilyen keresztnév nincs a névben: céges név*/
			WHEN name_ind_1 = 'first_name' AND name_ind_2 = 'first_name' AND name_ind_3 = '' AND name_ind_4 = '' THEN
...
FROM INVOICES_00d3 AS a
LEFT JOIN IN_megszolitasi_formak c
ON (a.gender = c.gender AND a.related_webshop = c.webshop)
;


ALTER TABLE INVOICES_00d4 ADD PRIMARY KEY (`sql_id`) USING BTREE;
ALTER TABLE INVOICES_00d4 ADD INDEX `real_name_clean` (`real_name_clean`) USING BTREE;
ALTER TABLE INVOICES_00d4 ADD INDEX `shipping_country_standardized` (`shipping_country_standardized`) USING BTREE;
ALTER TABLE INVOICES_00d4 ADD INDEX `gender` (`gender`) USING BTREE;



DROP TABLE IF EXISTS INVOICES_00d5;
CREATE TABLE INVOICES_00d5
SELECT DISTINCT a.*,
		REPLACE(REPLACE(REPLACE(REPLACE(full_name_raw,'/',''),'Dr ','Dr. '),'  !','!'),' !','!') AS full_name
FROM INVOICES_00d4 a
;


ALTER TABLE INVOICES_00d5
...
ALTER TABLE INVOICES_00d5 ADD PRIMARY KEY (`sql_id`) USING BTREE;
ALTER TABLE INVOICES_00d5 ADD INDEX `real_name_clean` (`real_name_clean`) USING BTREE;
ALTER TABLE INVOICES_00d5 ADD INDEX `shipping_country_standardized` (`shipping_country_standardized`) USING BTREE;
ALTER TABLE INVOICES_00d5 ADD INDEX `gender` (`gender`) USING BTREE;
