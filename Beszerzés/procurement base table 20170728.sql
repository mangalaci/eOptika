...
SELECT 	DISTINCT
		...
		p.item_gross_value - p.item_vat_value AS item_net_purchase_value_in_base_currency,
...
SELECT 	DISTINCT
		...
		o.item_gross_value - o.item_vat_value AS item_net_purchase_value_in_base_currency,
...
SELECT * 
FROM purchases 
WHERE item_sku = '1610000000017'
;



DROP TABLE IF EXISTS items_sold_by_supplier_201809;
CREATE TABLE IF NOT EXISTS items_sold_by_supplier_201809
SELECT DISTINCT b.erp_invoice_id, b.CT1_sku, b.item_net_purchase_price_in_base_currency, b.item_quantity, p.purchase_supplier_name
FROM `BASE_03_TABLE`b
LEFT JOIN 
(
SELECT CT1_SKU, purchase_created, purchase_supplier_name, MAX(item_net_purchase_price_in_base_currency) AS item_net_purchase_price_in_base_currency 
FROM BASE_TABLE_PRODUCT_SUPPLY
WHERE purchase_created >= '2012-01-01'
AND CT1_sku = '1DAM.85-0225'
GROUP BY CT1_SKU, purchase_created, purchase_supplier_name
) p
ON b.CT1_SKU = p.CT1_SKU
AND ROUND(b.item_net_purchase_price_in_base_currency,0) = ROUND(p.item_net_purchase_price_in_base_currency, 0)
WHERE b.origin = 'invoices'
AND b.created >= '2018-09-01'
AND b.created <= '2018-09-30'
AND b.related_division = 'Optika - HU'
GROUP BY CT1_SKU, purchase_created
;
...
SELECT a.CT1_SKU, a.purchase_created, a.purchase_supplier_name, MAX(a.item_net_purchase_price_in_base_currency) AS item_net_purchase_price_in_base_currency 
FROM BASE_TABLE_PRODUCT_SUPPLY a,
(
SELECT CT1_SKU, item_net_purchase_price_in_base_currency, MAX(purchase_created) AS purchase_created 
FROM BASE_TABLE_PRODUCT_SUPPLY
WHERE purchase_created >= '2012-01-01'
AND CT1_sku = '1DAM.85-0225'
GROUP BY CT1_SKU, item_net_purchase_price_in_base_currency
) b
WHERE a.CT1_SKU = b.CT1_SKU
AND a.purchase_created = b.purchase_created
AND a.purchase_created >= '2012-01-01'
AND a.CT1_sku = '1DAM.85-0225'
GROUP BY a.CT1_SKU, a.purchase_created, a.purchase_supplier_name
