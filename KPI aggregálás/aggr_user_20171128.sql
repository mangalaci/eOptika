...
SELECT
	...
	END	AS first_year_contact_lens_overuse_ratio, /* függ attól, hogy egy, vagy két szemre hord lencsét*/
		...
		CASE WHEN 
			(CASE 	WHEN DATE(created) = DATE(last_purchase) THEN time_order_to_dispatch END) > 100 
			 AND (CASE WHEN DATE(created) = DATE(last_purchase) THEN time_order_to_dispatch END)/AVG(time_order_to_dispatch) > 4 
		THEN 1 
		ELSE 0 
		END AS too_slow_dispatch,
