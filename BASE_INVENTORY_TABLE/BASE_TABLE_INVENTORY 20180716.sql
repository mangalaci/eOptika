...
DECLARE avg_items_sold_30days DECIMAL(6,2);
SET @avg_items_sold_30days = (SELECT avg_items_sold_30days FROM x_bar_30 WHERE related_warehouse = 'Cloud Fulfilment' AND CT1_sku = 'SYSTU10');
...
DROP TABLE IF EXISTS t_all_time;
CREATE TABLE IF NOT EXISTS t_all_time
SELECT 
		t.order_date,
		t.related_warehouse,
		t.CT1_sku,
...
ALTER TABLE t_all_time ADD INDEX (`order_date`) USING BTREE;
ALTER TABLE t_all_time ADD INDEX (`related_warehouse`) USING BTREE;
ALTER TABLE t_all_time ADD INDEX (`CT1_sku`) USING BTREE;



DROP TABLE IF EXISTS t_all_time_descartes;
CREATE TABLE IF NOT EXISTS t_all_time_descartes
SELECT c.date, t.related_warehouse, t.CT1_sku
FROM calendar_table c, t_all_time t
WHERE c.date = CURDATE()
GROUP BY c.date, t.related_warehouse, t.CT1_sku
;
