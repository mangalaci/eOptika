...
SELECT  
		...
		ROUND(i.actual_quantity*i.item_net_purchase_price_in_base_currency,0) AS net_inventory_value,
		a.CT2_sku,
		a.CT2_pack,
		a.CT3_product,
		a.CT3_product_short,
...
SELECT
		...
		case
			when b.related_warehouse = 'Teréz körút 41.' then 1
			when b.related_warehouse = 'Eszközök - Teréz körút 41.' then 5
			when b.related_warehouse = 'Anyagok - Teréz körút 41.' then 4
			when b.related_warehouse = 'Sérültek - Teréz körút 41.' then 6
			when b.related_warehouse = 'Nem találtuk! - Teréz körút 41.' then 8
			when b.related_warehouse = 'Teréz körút 50.' then 10
			when b.related_warehouse = 'Foglalás - Teréz körút 41.' then 9
			when b.related_warehouse = 'Foglalás - Teréz körút 50.' then 11
			when b.related_warehouse = 'Sérültek - Teréz körút 50.' then 12
			when b.related_warehouse = 'Nem találtuk! - Teréz körút 50.' then 13
			when b.related_warehouse = 'VW #01 - Teréz körút 41.' then 14
			when b.related_warehouse = 'Cloud Fulfilment' then 15
			when b.related_warehouse = 'Pueblo Torrequebrada' then 17
			when b.related_warehouse = 'iOttica.it Magazzino Amalfi' then 20
			when b.related_warehouse = 'Pueblo Torrequebrada < #001' then 18
			when b.related_warehouse = 'iOttica.it Magazzino Amalfi < #001' then 21
			when b.related_warehouse = 'GPSe (Expected returns)' then 23
			when b.related_warehouse = 'Cloud Fulfilment < #001' then 19
			when b.related_warehouse = 'GLS Trieste (Expected returns)' then 22
			when b.related_warehouse = 'Nem találtuk! - iOttica.it Magazzino Amalfi' then 27
			when b.related_warehouse = 'Sérültek - iOttica.it Magazzino Amalfi' then 26
			when b.related_warehouse = 'Sérültek - Pueblo Torrequebrada' then 24
			when b.related_warehouse = 'Nem találtuk! - Pueblo Torrequebrada' then 25
			when b.related_warehouse = 'FAN Courier (Expected returns)' then 33
			when b.related_warehouse = 'Táblás utca 39.' then 7
		end as stock_warehouse_id,
...
DROP TABLE IF EXISTS BASE_TABLE_INVENTORY_08;
CREATE TABLE IF NOT EXISTS BASE_TABLE_INVENTORY_08
SELECT item_sku, sum(quantity_still_waited) as quantity_still_waited
FROM outgoing_orders
WHERE created > '2017-01-02'
AND is_deleted = 'no'
GROUP by item_sku
HAVING sum(quantity_still_waited) > 0
;

ALTER TABLE BASE_TABLE_INVENTORY_08 ADD INDEX (`item_sku`) USING BTREE;
...
SELECT DISTINCT
		...
		END  AS buffer_level,
		r.quantity_still_waited,
		x.stock as supplier_stock
...
LEFT JOIN (select * from prc_products_kpi where status = 1) x
on m.stock_sku = x.sku
LEFT JOIN BASE_TABLE_INVENTORY_08 r
on m.stock_sku = r.item_sku
;
