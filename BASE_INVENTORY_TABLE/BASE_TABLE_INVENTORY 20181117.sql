...
SELECT item_sku, MAX(created) AS last_procurement_date
...
SELECT CT1_SKU, MAX(created) AS last_purchase_date 
FROM `BASE_03_TABLE`
...
ALTER TABLE last_purchase ADD INDEX (`last_purchase_date`) USING BTREE;
ALTER TABLE BASE_03_TABLE ADD INDEX (`created`) USING BTREE;



DROP TABLE IF EXISTS last_sale_price;
CREATE TABLE IF NOT EXISTS last_sale_price
SELECT b.CT1_SKU, AVG(b.item_net_sale_price_in_base_currency) AS  avg_last_sale_price
FROM BASE_03_TABLE b, last_purchase l
WHERE b.CT1_SKU = l.CT1_SKU 
AND b.created = l.last_purchase_date
AND b.origin = 'invoices'
GROUP BY b.CT1_SKU
;

ALTER TABLE last_sale_price ADD PRIMARY KEY (`CT1_SKU`) USING BTREE;



DROP TABLE IF EXISTS last_procurement_dt_sale_price;
CREATE TABLE IF NOT EXISTS last_procurement_dt_sale_price
SELECT p.*, s.avg_last_sale_price
FROM last_procurement p
LEFT JOIN last_sale_price s
ON p.item_sku = s.CT1_SKU
;

ALTER TABLE last_procurement_dt_sale_price ADD PRIMARY KEY (`item_sku`) USING BTREE;
...
SELECT  
		...
		ROUND(i.actual_quantity*l.avg_last_sale_price,2) AS net_inventory_value,
		a.CT2_sku,
		a.CT2_pack,
		a.CT3_product,
		a.CT3_product_short,
		...
		ROUND(s.sum_reserved_30days/s.sum_ordered_30days,2) AS service_level_30days,
		ROUND(s.sum_reserved_60days/s.sum_ordered_60days,2) AS service_level_60days,
		ROUND(s.sum_reserved_90days/s.sum_ordered_90days,2) AS service_level_90days,
		ROUND(s.sum_reserved_120days/s.sum_ordered_120days,2) AS service_level_120days,
		ROUND(s.sum_reserved_180days/s.sum_ordered_180days,2) AS service_level_180days,
		ROUND(s.sum_reserved_365days/s.sum_ordered_365days,2) AS service_level_365days
...
LEFT JOIN last_procurement_dt_sale_price l
...
SELECT  
		b.CT1_sku AS stock_sku,
		b.CT1_sku_name AS stock_name_hun,
		i.warehouse_id AS stock_warehouse_id,
		b.related_warehouse AS stock_warehouse,
		ROUND(i.actual_quantity) AS stock_actual_quantity,
		i.actual_quantity*l.avg_last_sale_price AS net_inventory_value,		
		a.CT2_sku,
		a.CT2_pack,
		a.CT3_product,
		a.CT3_product_short,
...
SELECT DISTINCT
		...
		m.service_level_30days,
		m.service_level_60days,
		m.service_level_90days,
		m.service_level_120days,
		m.service_level_180days,
		m.service_level_365days,
