UPDATE INVOICES_00_inc AS g
LEFT JOIN INVOICES_002_inc AS f2
...
WHERE CT2_pack in ('Szállítási díjak', 'Pénzbeszedési díjak', 'Pénzkezelési díjak', 'Utánvétkezelési díj')
...
CASE	WHEN (s.shipping_type = 'Sprinter' AND s.DESTINATIONS = 'HUNGARY') THEN
				...
				END
				WHEN s.shipping_type IN ('MPL')  AND s.pudo_type IN ('PP','PM','CS', 'HA', 'S"', 'M"', 'P"') 
						THEN n.shipping_country_standardized = s.DESTINATIONS AND n.pudo_type = s.pudo_type
				WHEN s.shipping_type IN ('MPL')  AND s.pudo_type NOT IN ('PP','PM','CS', 'HA', 'S"', 'M"', 'P"') 
						THEN n.shipping_country_standardized = s.DESTINATIONS AND s.pudo_type = 30
...
ON	n.date = e.date
where n.shipping_method <> 'Pickup in person'
GROUP BY n.order_id
;
...
DROP TABLE IF EXISTS INVOICES_00i_inc;
CREATE TABLE INVOICES_00i_inc
SELECT DISTINCT
	 t.*,
...
ALTER TABLE INVOICES_00i_inc ADD PRIMARY KEY (`sql_id`) USING BTREE;
...
UPDATE INVOICES_00_inc AS m
        LEFT JOIN
    INVOICES_00i_inc AS s ON m.sql_id = s.sql_id
SET
