...
SELECT DISTINCT
	...
	CASE 	WHEN SUBSTR(i.sku,1,2) = '17' THEN i.sku
			WHEN i.group_id in (391,392) THEN i.sku
			...
			WHEN j.general_name IS NULL AND i.group_id = 220 and INSTR(i.sku,"_") > 0 THEN LEFT(i.sku,INSTR(i.sku,"_")-1)
			WHEN j.general_name IS NULL AND i.group_id = 220 and INSTR(i.sku,"-") > 0 THEN LEFT(i.sku,INSTR(i.sku,"-")-1)
			WHEN j.parent_id in (81,332) or i.group_id in (358,368,395,455) THEN case when LOCATE('_',i.sku) > 1 then SUBSTRING_INDEX(i.sku,'_',2) else i.sku end
			WHEN j.parent_id in (367) THEN case when LOCATE('+',i.sku) > 1 then SUBSTRING_INDEX(i.sku,'+',2) else i.sku end
			WHEN j.parent_id in (367) THEN case when LOCATE('|',i.sku) > 1 then SUBSTRING_INDEX(i.sku,'|',2) else i.sku end
			WHEN j.parent_id in (367) THEN case when LOCATE('-',i.sku) > 1 then SUBSTRING_INDEX(i.sku,'-',2) else i.sku end
			WHEN i.group_id in (139) THEN i.name_hu
			ELSE 'Unknown'
	END AS CT2_sku,
    ...
    CASE 	WHEN x.sku is not null then x.model
			WHEN j.parent_id in (81,332) or i.group_id in (358,368,395,455) THEN
	...
	CASE 	WHEN SUBSTR(i.sku,1,2) = '17' THEN 'Service'
			...
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Levi%s%' THEN 'Levi`s'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Web WE%' THEN 'Web'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'More & More%' THEN 'More & More'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Havaianas%' THEN 'Havaianas'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Miu Miu%' THEN 'Miu Miu'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Juicy Couture%' THEN 'Juicy Couture'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Retrosuperfuture%' THEN 'Retrosuperfuture'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Tod%s%' THEN 'Tod`s'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Mercedes-Benz%' THEN 'Mercedes-Benz'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Hilton%' THEN 'Hilton'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Giorgio Armani%' THEN 'Giorgio Armani'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Emmanuelle Khanh%' THEN 'Emmanuelle Khanh'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Chloé%' THEN 'Chloé'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Mauboussin%' THEN 'Mauboussin'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'ill.i by Will.i.am%' THEN 'ill.i by Will.i.am'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Alain Mikli%' THEN 'Alain Mikli'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Armani Exchange%' THEN 'Armani Exchange'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Rodenstock%' THEN 'Rodenstock'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Garrett Leight%' THEN 'Garrett Leight'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'G-Star RAW%' THEN 'G-Star RAW'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'La Martina%' THEN 'La Martina'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Missoni%' THEN 'Missoni'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Ralph Lauren%' THEN 'Ralph Lauren'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Greater Than Infinity%' THEN 'Greater Than Infinity'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Arnette%' THEN 'Arnette'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Porsche Design%' THEN 'Porsche Design'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Kardashian Kollection%' THEN 'Kardashian Kollection'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Polo PH%' THEN 'Polo'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Iron Paris%' THEN 'Iron Paris'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Porsche Design%' THEN 'Porsche Design'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Love Moschino%' THEN 'Love Moschino'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Trussardi%' THEN 'Trussardi'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Coconuts Acetat Black%' THEN 'Coconuts Acetat Black'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Campbell Marson%' THEN 'Campbell Marson'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 's.Oliver%' THEN 's.Oliver'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Paul Smith%' THEN 'Paul Smith'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Liu Jo%' THEN 'Liu Jo'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Dunhill%' THEN 'Dunhill'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Nike %' THEN 'Nike'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'John Galliano%' THEN 'John Galliano'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'S.T. Dupont%' THEN 'S.T. Dupont'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'Vuarnet%' THEN 'Vuarnet'
			when i.group_id IN (220,334,335) AND i.name_hu LIKE 'CKY %' THEN 'CKY'
			/* szemüveglencsék */
...
/*	https://stackoverflow.com/questions/2742650/what-is-the-equivalent-of-regexp-substr-in-mysql */
	...
	current_date as product_introduction_dt
...
ON  i.sku = p.SKU
LEFT JOIN (select name, sku, base_sku, sku_ext, brand_code, model from prc_products_kpi where status = 1) x
on i.sku = x.sku
;


ALTER TABLE ab_cikkto_full ADD INDEX (`CT1_SKU`) USING BTREE;
ALTER TABLE ab_cikkto_full ADD INDEX (`CT2_pack`) USING BTREE;
ALTER TABLE ab_cikkto_full ADD INDEX `group_id` (`group_id`) USING BTREE;



DROP TABLE IF EXISTS INVOICES_CT2_pack_product_introduction_dt;
CREATE TABLE IF NOT EXISTS INVOICES_CT2_pack_product_introduction_dt
SELECT 	CT2_pack, 
		MIN(created) AS product_introduction_dt 
FROM INVOICES_00
GROUP by CT2_pack
;

ALTER TABLE INVOICES_CT2_pack_product_introduction_dt ADD INDEX `CT2_pack` (`CT2_pack`) USING BTREE;


UPDATE
  ab_cikkto_full AS C
  INNER JOIN INVOICES_CT2_pack_product_introduction_dt AS A 
  ON C.CT2_pack = A.CT2_pack
