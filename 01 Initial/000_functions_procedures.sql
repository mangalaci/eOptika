...
DELIMITER //
CREATE PROCEDURE IF NOT EXISTS CityUpdate(table_name VARCHAR(64), updated_table VARCHAR(64), address_type VARCHAR(32))
BEGIN

  IF address_type = 'shipping' THEN

	SET @SQL := CONCAT('UPDATE ', updated_table, ' AS m
...
SELECT DATEDIFF(max(DATE),min(DATE))+2 FROM exchange_rates_ext INTO n;
